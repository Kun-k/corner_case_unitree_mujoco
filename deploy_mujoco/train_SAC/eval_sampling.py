    ...
    for ep in range(int(episodes)):
        ...
        for step_idx in range(int(max_episode_steps)):
            ...
            if model is None:
                ...
                if classifier_gate is not None and action_meta["action_source"] == "sac":
                    gate_score = float(
                        classifier_gate.predict_proba(np.asarray(obs, dtype=np.float32), action_meta["action"])[0]
                    )
                    if gate_score <= float(classifier_threshold):
                        fallback_action, fallback_density = _sample_uniform_action_with_density(env.action_space, rng)
                        action_meta["action"] = fallback_action
                        action_meta["action_source"] = "uniform_gate_fallback"
                        action_meta["action_log_prob"] = None
                        action_meta["action_prob_density"] = float(fallback_density)
                        action_meta["uniform_density"] = float(fallback_density)
                        action_meta["sac_density"] = None
                    action_meta["action_gate_score"] = gate_score
                else:
                    action_meta["action_gate_score"] = None
            ...
            transition = {
                ...
                "action_gate_score": action_meta.get("action_gate_score"),
                "reward": float(reward),
