...
class Go2Controller:
    def __init__(self, config_file):
        ...
        self.policy2model = self.config["mapping_joints"]
    ...
    def get_observation(self, d):
        ...
        elif self.num_obs == 45:
            obs[:3] = ang_vel
            obs[3:6] = gravity_orientation
            obs[6:9] = self.cmd * self.cmd_scale
            obs[9:21] = qj
            obs[21:33] = dqj
            obs[33:45] = self.action_policy_prev
        else:
    ...
    def test_run(self):  # 独立启动仿真运行，仿真文件路径来源于config
        counter = 0
